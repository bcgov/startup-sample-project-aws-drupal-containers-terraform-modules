...
resource "aws_iam_role_policy" "ecs_task_execution_dbsecret" {
  name = "ecs_task_execution_dbsecret"
    ...
    "Statement": [
      {
          ...
          "Resource": "${aws_rds_cluster.mysql.arn}"
        }
    ]
  }
  EOF  
...
resource "aws_iam_role_policy" "sample_efs" {
  name = "sample_efs"
  role = aws_iam_role.sample_app_container_role.id
    ...
    "Statement": [
      {
          ...
          "Condition": {
            "StringEquals": {
              "aws:TagKeys/Name": "${aws_ecs_cluster.main.name}"
            }
          }
        }
    ]
...
resource "aws_iam_role" "sample_app_autoscaling" {
  name = "sample_app_autoscaling_container_role"
